...
func Serve(
	...
	mysqlDbEditor := mysqlDb.Editor()
	var userCount int
	mysqlDbEditor.VisitUsers(func(*mysql_db.User) {
		userCount += 1
	})
	privsExist := userCount != 0
	if userSpecified {
		superuser := mysqlDb.GetUser(mysqlDbEditor, config.ServerUser, "%", false)
		if userSpecified && superuser == nil {
			mysqlDb.AddSuperUser(mysqlDbEditor, config.ServerUser, "%", config.ServerPass)
		}
	} else if !privsExist {
		mysqlDb.AddSuperUser(mysqlDbEditor, defaultUser, "%", defaultPass)
	}
	mysqlDbEditor.Close()
	...
	mysqlDbEditor = mysqlDb.Editor()
	mysqlDb.AddSuperUser(mysqlDbEditor, LocalConnectionUser, "localhost", lck.Secret)
	mysqlDbEditor.Close()
